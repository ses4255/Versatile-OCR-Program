...
def run_docker_container(input_dir, output_dir, credentials_dir, image_name="shit"):
    ...
    try:
        ...
        cmd_list = [
            ...
            "-v", f"{credentials_dir}:/app/credentials",
            "-e", "PDF_FOLDER=/app/input",
            "-e", "OUTPUT_FOLDER=/app/output",
            "-e", "PYTHONUNBUFFERED=1",
            "-e", f"GOOGLE_APPLICATION_CREDENTIALS=/app/credentials/Google_Vision_S.Account.json",
        ...
        logger.info(f"Running Docker container: {' '.join(cmd_list)}")
